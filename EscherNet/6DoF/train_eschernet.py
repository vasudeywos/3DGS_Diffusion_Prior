...
from colmap_scene_dataset import ColmapSceneData
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--initial_eschernet_checkpoint",
        type=str,
        default=None,
        help=(
            "Optional EscherNet .bin/.pth checkpoint used to initialize the trainable "
            "UNet/image encoder before COLMAP adaptation. Use this when the official "
            "checkpoint is not a full Diffusers pipeline directory."
        ),
    )
    parser.add_argument(
        "--revision",
    ...
    parser.add_argument(
        "--dataset_type",
        choices=["objaverse", "colmap"],
        default="objaverse",
        help="Use original Objaverse layout or COLMAP open-scene layout.",
    )
    parser.add_argument(
        "--scaffold_root",
        type=str,
        default=None,
        help="Path to Scaffold-GS-main for COLMAP readers.",
    )
    parser.add_argument("--images_name", type=str, default="images")
    parser.add_argument(
        "--exclude_scenes",
        type=str,
        default="",
        help="Comma-separated COLMAP scene folder names to exclude, e.g. bicycle.",
    )
    parser.add_argument("--colmap_window_radius", type=int, default=8)
    parser.add_argument("--samples_per_scene", type=int, default=512)
    parser.add_argument(
        "--high_noise_shift",
        action="store_true",
        help="Bias diffusion training timesteps toward higher noise levels.",
    )
    parser.add_argument(
        "--high_noise_min_fraction",
        type=float,
        default=0.35,
...
def main(args):
    ...
    if args.initial_eschernet_checkpoint is not None:
        checkpoint_path = Path(args.initial_eschernet_checkpoint).expanduser()
        logger.info(f"Loading initial EscherNet checkpoint from {checkpoint_path}")
        ...
        if isinstance(checkpoint, dict) and "unet" in checkpoint:
            missing, unexpected = unet.load_state_dict(checkpoint["unet"], strict=False)
            logger.info(f"Loaded checkpoint['unet']; missing={len(missing)}, unexpected={len(unexpected)}")
            loaded_any = True
        elif isinstance(checkpoint, dict):
            unet_state = _strip_prefix(checkpoint, "unet.")
            if len(unet_state) > 0:
                missing, unexpected = unet.load_state_dict(unet_state, strict=False)
                logger.info(f"Loaded unet.* weights; missing={len(missing)}, unexpected={len(unexpected)}")
                loaded_any = True

        if isinstance(checkpoint, dict) and "image_encoder" in checkpoint:
            missing, unexpected = image_encoder.load_state_dict(checkpoint["image_encoder"], strict=False)
            logger.info(f"Loaded checkpoint['image_encoder']; missing={len(missing)}, unexpected={len(unexpected)}")
        elif isinstance(checkpoint, dict):
            image_encoder_state = _strip_prefix(checkpoint, "image_encoder.")
            if len(image_encoder_state) > 0:
                missing, unexpected = image_encoder.load_state_dict(image_encoder_state, strict=False)
                logger.info(f"Loaded image_encoder.* weights; missing={len(missing)}, unexpected={len(unexpected)}")

        if not loaded_any:
            missing, unexpected = unet.load_state_dict(checkpoint, strict=False)
            logger.info(f"Loaded checkpoint directly into UNet; missing={len(missing)}, unexpected={len(unexpected)}")
    ...
    if args.dataset_type == "objaverse":
        ...
        if args.scaffold_root is None:
            raise ValueError("--scaffold_root is required with --dataset_type colmap.")
        train_dataset = ColmapSceneData(
            ...
            T_out=T_out,
            fix_sample=False,
            window_radius=args.colmap_window_radius,
            samples_per_scene=args.samples_per_scene,
        )
        train_log_dataset = ColmapSceneData(
            root_dir=args.train_data_dir,
            image_transforms=image_transforms,
            scaffold_root=args.scaffold_root,
            images_name=args.images_name,
            ...
            fix_sample=True,
            window_radius=args.colmap_window_radius,
            samples_per_scene=max(16, min(args.samples_per_scene, 64)),
        )
        validation_dataset = ColmapSceneData(
            root_dir=args.train_data_dir,
            image_transforms=image_transforms,
            scaffold_root=args.scaffold_root,
            images_name=args.images_name,
    ...
    if accelerator.is_main_process:
        ...
        run_name = (
            args.output_dir.split("logs_", 1)[1]
            if "logs_" in args.output_dir
            else Path(args.output_dir).name
        )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, image_encoder):
                ...
                if args.high_noise_shift:
                    low = int(
                        args.high_noise_min_fraction
                        * noise_scheduler.config.num_train_timesteps
                    )
                    low = max(0, min(low, noise_scheduler.config.num_train_timesteps - 1))
