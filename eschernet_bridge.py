...
def main():
    ...
    for clip in job["clips"]:
        ...
        for image_array, teacher in zip(result, teachers):
            filename = (
                f"clip_{teacher['clip_index']:02d}_"
                f"frame_{teacher['frame_index']:02d}.png"
            )
            image = (np.clip(image_array, 0.0, 1.0) * 255.0).round().astype(np.uint8)
